...
function LoginPage() {
  ...
  const handleSubmit = (event) => {
    event.preventDefault();
    const newData = response.find(
      (item) => item.email === values.email && item.pass === values.password
    );
    ...
    <>
      {wrong ? (
        <h4 style={{ color: "red", margin: "auto" }}>
          Invalid Credentials, if you have not registered then click on{" "}
          <span style={{ color: "black" }}>Create one</span> to register
        </h4>
      ) : null}
      ...
      <div className={classes.root}>
        <form onSubmit={handleSubmit}>
          ...
          <Input className={classes.inputSubmit} type="submit" value="Log In" />
        </form>
